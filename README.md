...
- data:
	...
	- data/interactions/raw/raw_interactions.csv: CSV file containing the annotated interactions between each phage (n=96) and each bacterial strain (excluding the phage isolation strains which were assessed separately, n=369), at each phage dilution (3 dilutions testested) and at reach replicate (three replicates). These raw annotations are not aggregated across dilutions and across replicates and were used to generate the interaction matrix used in the article (`data/interactions/interaction_matrix.csv`). Each line in this table corresponds to one (bacteria, phage, phage dilution, replicate) tuple and contains the following information: `bacteria` (bacterial strain name), `bacteria_index` (integer index that is unique to each strain), `phage` (phage name), `image` (name of the image containing the interaction), `replicate` (index of the replicate, from 1 to 3), `plate` (Each image contains four petri dishes corresponding to the same bacterial strain and containing 96 interactions tested each. Petri dishes are numerotated A (top-left), B (top-right), C (bottom-left), D (bottom-right)), `log_dilution` (Log-phage dilution which is either 0 (replicates 1, 2, 3), -1 (replicates 2 and 3), -2 (replicates 1, 2, 3) and -4 (replicate 1)), `X` and `Y` (within one petri dish, 96 interactions are tested, `X` corresponds to the 0-based index of the row containing the interaction and `Y` corresponds to the 0-based index of the column; row and column indexing start are the top-left of each plate), `score` (Annotated score of the interaction. Annotated score can either be 0 (no lytic interaction), 1 (Lytic interaction), or 'n' (Result not interpretable in the image)). 
- dev:
