...
- [x] Lock ST03 split as v1 benchmark and add bootstrap CIs for all metrics Model: `gpt-5.4-mini`.
...
- [x] Before/after comparison of v0 vs v1 with error bucket analysis Model: `gpt-5.4-mini`.
...
- [x] Compute SHAP explanations for per-pair and global feature importance Model: `gpt-5.4`.
  ...
  - Concrete recommendation of which feature blocks to keep in final v1 model, based on SHAP evidence and TG03 ablation
    results
- [x] Run feature-subset sweep to find best block combination for top-3 ranking Model: `gpt-5.4`.
...
- [ ] Delete label-leaked features from the feature pipeline Model: `gpt-5.4`.
...
- [ ] Retrain, recalibrate, and re-run SHAP and ablation on the clean feature set Model: `gpt-5.4`.
...
- [ ] Re-run downstream tracks and verify end-to-end pipeline Model: `gpt-5.4`.
...
- [ ] Investigate non-leaky features that close the calibration gap Model: `gpt-5.4`.
...
- [x] Add explained recommendations with calibrated P(lysis), CI, and SHAP features Model: `gpt-5.4-mini`.
...
- [x] Define harmonization protocol for Tier A datasets Model: `gpt-5.4`.
- [x] Tier B weak-label ingestion: Virus-Host DB and NCBI Virus/BioSample metadata Model: `gpt-5.4-mini`.
- [x] Define confidence tiers for external labels Model: `gpt-5.4`.
- [x] Integrate external data as non-blocking enhancer: internal-only baseline must remain runnable Model: `gpt-5.4`.
- [x] Run strict ablations in sequence: internal-only -> +VHRdb -> +BASEL -> +KlebPhaCol -> +GPB -> +Tier B Model:
      `gpt-5.4-mini`.
- [x] Track incremental lift and failure modes by datasource and confidence tier Model: `gpt-5.4-mini`.
...
- [x] One command to regenerate all v1 outputs from raw data Model: `gpt-5.4-mini`.
...
- [x] Freeze environment specs and seeds for v1 benchmark run Model: `gpt-5.4-mini`.
