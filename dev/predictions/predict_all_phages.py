...
for p in phage_features.index:
    ...
    phage_host_features = pd.merge(
        phage_feat,
        bact_features.filter(regex="(ST_Warwick|O-type|H-type|ABC_serotype)", axis=1),
        left_on="Phage_host",
        right_index=True,
    ).rename(
        {
    ...
    if "ABC_serotype" in bact_features.columns:
        if not p.startswith("LF110"):
            if "ABC_host" in interaction_with_features.columns:
                interaction_with_features["same_ABC_as_host"] = interaction_with_features["ABC_serotype"] == interaction_with_features["ABC_host"]
                interaction_with_features = interaction_with_features.drop("ABC_host", axis=1)
